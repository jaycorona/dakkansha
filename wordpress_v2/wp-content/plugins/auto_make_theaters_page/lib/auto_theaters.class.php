...
class AutoTheater {
	...
	private function getLinksHtml($array) {
		...
		// connect the results
		$str_buffer = "\t\t<div align=\"center\" class=\"arealist paddingB30\">";
	...
	private function getTitleHtml($array) {
		...
		$str_buffer .= "\t\t\t<p class=\"area\">".$array[0]."</p>\n";
	...
	function getHeaderHtml($array) {
		...
		$str_buffer = "\t\t\t<article class=\"top-header clearfix\" >\n";
		$e=1;
	...
	function getRowHtml($array) {
		...
		//$date = DateTime::createFromFormat('m-d-Y', $array[3]);
	...
	private function combineSections($array){
		...
		for ($i=0;$i<count($this->title_list);$i++) {
		
			$title = $this->title_list[$i];
		
			$title = preg_replace("/#(.*)/", "$1", $title);
		
			$str_buffer .= "\t<div id=\"".$title."\" class=\"wrap\">\n";
			...
			$str_buffer .= "\t\t<section class=\"article-container list-all\">\n";
			$str_buffer .= implode("\n", $rows);
			$str_buffer .= "\t\t</section>\n";
			$str_buffer .= "\t</div>\n";
		}
