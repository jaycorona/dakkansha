...
class Share_Twitter extends Sharing_Source {
	...
	public function get_display( $post ) {
		...
		if ( $this->smart ) {
			...
			return $this->get_link( get_permalink( $post->ID ), _x( 'Twitterでシェア', 'share to', 'jetpack' ), __( 'Click to share on Twitter', 'jetpack' ), 'share=twitter', 'sharing-twitter-' . $post->ID );
		}
	}
	...
	public function process_request( $post, array $post_data ) {
		...
		$post_title = "【{$site_name}】" . $post_title;
...
class Share_Facebook extends Sharing_Source {
	...
	public function get_name() {
		return __( 'Facebookでシェア', 'jetpack' );
	}
	...
	public function get_display( $post ) {
		...
		return $this->get_link( get_permalink( $post->ID ), _x( 'Facebookでシェア', 'share to', 'jetpack' ), __( 'Share on Facebook', 'jetpack' ), 'share=facebook', 'sharing-facebook-' . $post->ID );
	}
...
class Share_GooglePlus1 extends Sharing_Source {
	...
	public function get_display( $post ) {
		...
		if ( $this->smart ) {
			...
			return $this->get_link( get_permalink( $post->ID ), _x( 'Google+でシェア', 'share to', 'jetpack' ), __( 'Click to share on Google+', 'jetpack' ), 'share=google-plus-1', 'sharing-google-' . $post->ID );
		}
	}
